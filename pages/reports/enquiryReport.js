...
const enquiryNew = async (req, res) => {
    try {
        const html = fs.readFileSync("quotation1.html", 'utf-8');
        // let enquiryNo = "MM/QN/22-23/00006";
        let { enquiryNo } = req.query;
        if (enquiryNo == undefined || enquiryNo.length == 0) {
            return res.status(200).json({ isSuccess: true, data: null, message: "Please provide enquiry no" });
        }
        // console.log(enquiryNo);
        enquiryNoFormat = enquiryNo.replace(/bhbcbs|\/|bhjcbc/g, '-');
        let exist = false;
        ...
        //     // console.log(files);
        //     exist = files.includes(enquiryNoFormat + '.pdf');
        // });
        // console.log(enquiryNoFormat);
        // return;
        let getPdfs = await listPdfs();
        for (i = 0; i < getPdfs.length; i++) {
            if (getPdfs[i].Key == `${enquiryNoFormat}.pdf`) {
                exist = true;
            }
        }
        // if (exist) {
        //     return res.status(200).json({ isSuccess: true, data: `https://s3.ap-south-1.amazonaws.com/pdf.marudhar.app/${enquiryNoFormat}.pdf`, name: `${enquiryNoFormat}.pdf`, message: "Pdf already exist" });
        ...
        console.log("getQuotation => ",getQuotation[0]);
        if (getQuotation.length > 0) {
            let quotationDate = new Date(getQuotation[0].quotation_date);
            let dateFormat = moment.unix(getQuotation[0].enquiry_details[0].enquiry_date).format("DD/MM/YYYY");
            console.log("dateFoemat => ",dateFormat);
            let quotationItems = getQuotation[0].enquiry_item_details
            ...
            for (i = 0; i < quotationItems.length; i++) {
                ...
                let objIs = {
                    sno: i + 1, qty: quotationItems[i].quantity, disc_value
                        : disc_valueIs, mrp: mrpIs, per: quotationItems[i].uom_name, disc: quotationItems[i].disc_percentage, gst: quotationItems[i].gst_percentage
                }
                arrIs.push({ ...objIs, ...{ name: nameIs, img: imgIs } });
            }
            console.log("checkpoint1 => ", arrIs)
            ...
            console.log("date => ",obj.curdate);
            // return res.render('quotation1', obj);
            const pdf = await printPDF(obj, true, enquiryNoFormat);
            ...
            //         products: obj
            //     },
            //     path: path.join(__dirname, '../', '../', 'public', `${enquiryNoFormat}.pdf`)
            // }
            let uploadpdfIs = "";
            uploadpdfIs = await uploadPdf(`${enquiryNoFormat}`);
            // return res.send(pdf)
            ...
            // return res.download(path.join(__dirname, '../', '../', 'public', `${enquiryNoFormat}.pdf`))
            return res.status(200).json({ isSuccess: true, data: uploadpdfIs, name: `${enquiryNoFormat}.pdf`, message: "Pdf Generated Successfully" });
        }
...
async function printPDF(obj, isQuotation, name) {
    ...
    if (isQuotation == true) {
        pageContent = await ejs.renderFile(path.join(__dirname, '../', '../', 'views', 'quotation3.ejs'), {
            curdate: obj.curdate,
...
const salesNew = async (req, res) => {
    try {
        ...
        if (getQuotation.length > 0) {
            ...
            let quotationItems = getQuotation[0].enquiry_item_details
            // let quotationCharge = getQuotation[0].quotation_other_charges;
            let enquiryNoIs = getQuotation[0].quotation_no[0];
            let finalAmount = 0;
            quotationItems.forEach(object => {
                if ((typeof object.net_value) == 'string') {
                    finalAmount += parseFloat(parseFloat(object.net_value).toFixed(2));
            ...
            const pdf = await printPDF(obj, false, enquiryNoFormat);
            // res.contentType("application/pdf");
            // return res.send(pdf);
            // const document = {
            //     html: html,
...
const deliveryNew = async (req, res) => {
    try {
        ...
        if (getDelivery.length > 0) {
            ...
            let enquiryNoIs = getDelivery[0].quotation_no[0];
            let finalAmount = 0;
...
module.exports = { enquiryNew, salesNew, deliveryNew }