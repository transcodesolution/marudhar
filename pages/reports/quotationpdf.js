...
const quotationNew = async (req, res) => {
    try {
        ...
        console.log(getQuotation,"-----ehdfk----------------------");
            ...
            for (i = 0; i < quotationItems.length; i++) {
                ...
                let objIs = {
                    sno: i + 1, qty: quotationItems[i].quantity, disc_value
                        : disc_valueIs, mrp: mrpIs, per: quotationItems[i].uom_name, disc: quotationItems[i].disc_percentage, gst: quotationItems[i].gst_percentage, gst_value: gstIs, prprice: net_valueIs
                }
