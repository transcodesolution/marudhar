...
const { primary_group, account_nature, ledger_group, ledger_account, tax_master, charges, bank_master } = require("../../modals/MasterAccounts");
...
const AccountMasterInsert = async (req, res) => {
  ...
  return res.status(200).json({ _id: insertedMaster._id, ledger_account_id: insertedMaster?.ledger_account_id ? insertedMaster?.ledger_account_id : 0 });
};
...
const viewPrimaryGroup = async (req, res) => {
  ...
  // Details by ID
  if (primary_group_id) { condition = { ...condition, primary_group_id: primary_group_id } }

  // Details by Name
  if (primary_group) { condition = { ...condition, primary_group: { '$regex': '^' + primary_group + '$', $options: 'i' } } } // Matching exact text but incase-sensitive
  ...
  await primary_group.find(condition, (err, primaryGroupData) => {
    if (err) {
    ...
    if (primaryGroupData) {
      ...
      let returnDataArr = primaryGroupData.map(data => ({
        ...
        //     }
        ...data["_doc"], can_edit: false, can_delete: true, can_activate: true, can_deactivate: false
      }))
  ...
  }).select((short_data === true && { "primary_group_id": 1, "primary_group_id": 1, "_id": 0 }));
};
...
const view_ledger_balance = async (req, res) => {
  ...
  let ledger_balance_data = await ledger_account.aggregate(
    [
      { $match: condition },
      { $project: { ledger_account_id: 1, ledger_account: 1, opening_balance: 1, dr_cr_status: 1, _id: 0 } },
      {
        $lookup:
        {
          from: 't_200_journals',
              ...
              $project: {
                ...
                debit_balance: {
                  $cond: {
                    ...
                    then: { $toDouble: "$journal_details.amount" }, else: 0
                  }
                },
                credit_balance: {
                  $cond: {
                ...
                debit_balance: {
                  $cond: {
                    if: {
                      $or: [{
                        $and: [{ $in: ["$receipt_payment_type", ["R", "BP"]] },
                        { $lt: ["$voucher_date", to_date] }, { $eq: ["$bank_id", ledger_account_id] }]
                      },
                      {
                        $and: [{ $in: ["$receipt_payment_type", ["P", "BR"]] },
                        { $lt: ["$voucher_date", to_date] }, { $eq: ["$ledger_account_id", ledger_account_id] }]
                      }]
                    },
                    then: { $toDouble: "$amount" }, else: 0
                  }
                },
                credit_balance: {
                  $cond: {
...
const insertLedgerStorage = async (req, res) => {
  let condition = {};
  const from_date = moment(moment("2022-04-01").format("x"), "x").unix();
  const to_date = moment(moment("2023-03-31").endOf('day').format("x"), "x").unix()
  const ledger_account_id = req.body.ledger_account_id;
  const ledger_account_name = req.body.ledger_account;
  // const ledgerAccountOpt = req.body.ledgerAccountOpt;
                  ...
                  then: {
                    $switch: {
                      branches: [
                        {
                          case: { $eq: [dr_cr, 2] },
                          then: { $subtract: [{ $abs: "$closingBalance" }, Math.abs(amount)] }
                        },
                        {
                          case: { $eq: [dr_cr, 1] },
                          then: { $subtract: [{ $abs: "$closingBalance" }, -Math.abs(amount)] }
                    ...
                    $switch: {
                      branches: [
                        ...
                        },
                        {
                          ...
                          then: {
                            $cond: {
                              if: { $gt: [{ $subtract: [{ $abs: "$closingBalance" }, -Math.abs(amount)] }, 0] },
                              then: "Cr",
                              else: "Dr"
                            }
                          }
...
module.exports = { insertLedgerStorage, AccountMasterInsert, viewPrimaryGroup, view_ledger_balance, editLedgerStorage };
